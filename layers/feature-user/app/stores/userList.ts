...
type State = {
  query: Omit<MockUserSearchCriteria, 'page' | 'pageSize'>
  items: MockUser[]
  total: number
  loading: boolean
  error: string | null
...
export const useUserListStore = defineStore('featureUsers.userList', {
  state: (): State => ({
    query: initialQuery(),
    items: [],
    total: 0,
  ...
  actions: {
    ...
    async search(override?: Partial<State['query']>) {
      ...
      try {
        ...
        const criteria: MockUserSearchCriteria = {
          ...this.query,
          page: this.page,
          pageSize: this.pageSize,
        }
        const res: MockPaged<MockUser> = await mockUserRepository.search(criteria)
