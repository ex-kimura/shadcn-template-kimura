...
import type { InspectionCategory, InspectionTemplate } from '@/types/inspection'
...
const allRows = ref<InspectionTemplate[]>([])
...
const categoryNameMap = computed(() => {
  ...
  for (const row of categories.value) map.set(row.id, row.name)
  return map
})
...
async function load() {
  ...
  try {
    const [c, t] = await Promise.all([
      mockInspectionRepository.listCategories(false),
      mockInspectionRepository.listTemplates(false),
    ])
    categories.value = c
    allRows.value = t
  }
        ...
        <template #result="{ toggleLeftPanel, openFilter }">
          <div class="h-full min-h-0">
            ...
            <div class="space-y-2 p-2 md:hidden">
              ...
              >
                <div>ID: {{ row.id }}</div>
                <div>テンプレート名: {{ row.name }}</div>
                <div>カテゴリ: {{ categoryNameMap.get(row.categoryId) ?? row.categoryId }}</div>
                <div>しきい値1: {{ row.thresholdOk }}</div>
                <div>しきい値2: {{ row.thresholdReview }}</div>
                <div class="mt-1">
                  <Badge :variant="row.isEnabled ? 'success' : 'secondary'">
                    {{ row.isEnabled ? '有効' : '無効' }}
                  </Badge>
